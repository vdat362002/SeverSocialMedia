  ...
  async (req, res, next) => {
    try {
      ...
      await post
        .populate({
          path: "author",
          select: "profilePicture username fullname firstname lastname name",
        })
        .execPopulate();
...
router.get("/v1/:username/posts", isAuthenticated, async (req, res, next) => {
  try {
...
router.post("/v1/post/share", isAuthenticated, async (req, res, next) => {
  try {
    const { idPost } = req.body;
    const oldPost = await Post.findOne({ _id: idPost });
    if (!oldPost) {
    ...
    const newPostData = {
      ...
      privacy: "public",
      oldInfo: {
        user: oldPost._author_id,
        createdAt: oldPost.createdAt,
      },
    };
    delete newPostData._id;
    delete newPostData.createdAt;
    delete newPostData.updatedAt;
    let post = new Post(newPostData);
